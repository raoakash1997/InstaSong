...
const getArtistForUsername = async(userName) => {
  try{
    return await axios.post(API_URL + "getArtistForUsername", {userName})
  }catch(error){
    console.log(error)
  }
}

const getSongsForFeed = async(userName) => {
  try{
    return await axios.post(API_URL + "getSongsForFeed", {userName})
  }catch(error){
    console.log(error)
  }
}
